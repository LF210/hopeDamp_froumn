...
export default class myMessage extends Component {
  ...
  componentDidMount() {
    ...
    socket.on("value", data => {
      ...
      if (
        JSON.parse(JSON.stringify(data)).user_id !== this.state.chatingUser.id
      ) {
        return false;
      }
      // 否则将新消息自动同步到页面中
  ...
  chatViewScrollToBottom = () => {
    // 等待数据挂载完毕后再滚动
    setTimeout(() => {
      document.querySelector(".chat-view").scrollTop = document.querySelector(
        ".chat-view"
      ).scrollHeight;
    }, 30);
  };
    ...
    if (this.state.historyChatUser) {
      ...
      this.state.historyChatUser.forEach((item, index) => {
        chatUserData.push(
          <div
            key={index}
            className="chatUserBox"
            onClick={() => this.changeChat(item, index)}
          >
        ...
        if (item.user_id === jwt.decode(sessionStorage.getItem("token")).id) {
          chatMsg.push(
            <div key={index} className="myMsg">
              <div className="chatUserBox-avatar">
                {item.avatar && (
                  <img src={item.avatar} className="user-avatar" />
                )}
                {!item.avatar && (
                  <img
                    style={{ width: "30px", height: "30px" }}
