...
export default class publish extends Component {
    ...
    componentDidMount() {
        ...
        editor.customConfig.onchange = (html) => {
            ...
            if (this.state.publishType === 0) {
                ...
                localStorage.setItem('publish_id', jwt.decode(sessionStorage.getItem('token')).id)
            }
        }
        // 富文本编辑器的配置项
        ...
        if (sessionStorage.getItem('edit-title') !== null) {
            ...
            // 单选分类同步数据
            document.querySelector('select').value = sessionStorage.getItem('edit-classify_id')
            // 同步数据结束后清空缓存
            ...
            localStorage.getItem('publish_id') === jwt.decode(sessionStorage.getItem('token')).id &&
            (localStorage.getItem('publish_value') !== null || localStorage.getItem('publish_classify') !== null || localStorage.getItem('publish_title') !== null)
        ) {
            // 发布新话题（存在草稿）
            this.setState({
                publishType: 0
            ...
            confirm({
                ...
                onOk: () => {
                    ...
                    document.querySelector('select').value = localStorage.getItem('publish_classify')
                },
                oncancel: () => {
                    // 不导入草稿箱则清空草稿箱
                    localStorage.clear()
            ...
            // 发布新话题（没有草稿）
        } else if (localStorage.getItem('publish_id') !== jwt.decode(sessionStorage.getItem('token')).id) {
            // 清空草稿箱内容
            localStorage.clear()
            // 将状态设为发布话题
            this.setState({
    ...
    render() {
        const classify = []
        this.state.classify_arr.forEach((item, index) => {
            classify.push(
                ...
                <div className="center-nav">
                    <div className="center-nav-left">
                        <div className="publish-card">
                            <div className="publish-card-head">{(this.state.publishType === 0 && '发布新话题') || '修改话题'}</div>
                            <div className="publish-card-body">
                                ...
                                <a href="/" className="btn" style={{ marginTop: '1rem' }} onClick={this.publishClick}>
                                    发布
                                </a>
                            </div>
                        </div>
